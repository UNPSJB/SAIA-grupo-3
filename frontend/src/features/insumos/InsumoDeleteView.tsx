...
interface InsumoDeleteViewProps {
  insumo: Insumo;
  onConfirmarEliminar: (id: number) => Promise<void>;
  onCancelar: () => void;
}

export function InsumoDeleteView({ insumo, onConfirmarEliminar, onCancelar }: InsumoDeleteViewProps) {
  ...
  return (
    <Card className="border-danger shadow-sm mx-auto" style={{ maxWidth: '650px' }}>
      <Card.Header className="bg-danger text-white d-flex align-items-center gap-2 py-3">
        ...
        <h5 className="mb-0">Eliminar Insumo</h5>
      </Card.Header>
      <Card.Body className="p-4">
        ...
        <p className="text-secondary fs-6">¿Estás seguro de que deseas eliminar este insumo del sistema?</p>
