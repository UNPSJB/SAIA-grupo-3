...
interface ElementoDeleteViewProps {
  elemento: Elemento;
  onConfirmarEliminar: (id: number) => Promise<void>;
  onCancelar: () => void;
}

export function ElementoDeleteView({
  elemento,
  onConfirmarEliminar,
  onCancelar,
}: ElementoDeleteViewProps) {
  ...
  const handleEliminar = async () => {
    ...
    try {
      await onConfirmarEliminar(elemento.id);
    } catch (err: unknown) {
      setErrorBackend(
      ...
      <Card.Body className="p-4">
        ...
        <div className="bg-light p-3 rounded border mb-4">
          ...
          <div className="row mb-2">
            ...
            <span className="col-8">{elemento.frecuencia_recambio ? `${elemento.frecuencia_recambio} días` : 'No definido'}</span>
          </div>
        </div>
