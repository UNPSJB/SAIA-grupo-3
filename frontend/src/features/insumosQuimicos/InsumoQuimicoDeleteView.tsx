...
interface InsumoQuimicoDeleteViewProps {
  insumo: InsumoQuimico;
  onConfirmarEliminar: (id: number) => Promise<void>;
  onCancelar: () => void;
}

export function InsumoQuimicoDeleteView({
  insumo,
  onConfirmarEliminar,
  onCancelar,
}: InsumoQuimicoDeleteViewProps) {
  const [eliminando, setEliminando] = useState(false);
  const [errorBackend, setErrorBackend] = useState<string | null>(null);

  const etiquetaTipo =
    TIPOS_QUIMICOS.find((t) => t.value === insumo.tipo_quimico)?.label ?? insumo.tipo_quimico;

  const handleEliminar = async () => {
    setEliminando(true);
    setErrorBackend(null);
    try {
      await onConfirmarEliminar(insumo.id);
    } catch (err: unknown) {
      setErrorBackend(err instanceof Error ? err.message : 'Error al procesar la baja del insumo químico.');
      setEliminando(false);
    }
  };
  ...
  return (
    <Card className="border-danger shadow-sm mx-auto" style={{ maxWidth: '650px' }}>
      <Card.Header className="bg-danger text-white d-flex align-items-center gap-2 py-3">
        ...
        <h5 className="mb-0">Dar de Baja Insumo Químico</h5>
      </Card.Header>
      <Card.Body className="p-4">
        ...
        <p className="text-secondary fs-6">
          ¿Estás seguro de que deseas dar de baja este insumo químico del inventario activo?
        </p>
        <div className="bg-light p-3 rounded border mb-4">
          <div className="row mb-2">
            <span className="col-4 text-muted fw-semibold">ID:</span>
            <span className="col-8">
              <Badge bg="secondary">#{insumo.id}</Badge>
            </span>
          </div>
          <div className="row mb-2">
          ...
          <div className="row mb-2">
            ...
            <span className="col-8">{etiquetaTipo}</span>
          </div>
          <div className="row">
            <span className="col-4 text-muted fw-semibold">Cantidad:</span>
            <span className="col-8">
              {insumo.cantidad} {insumo.unidadMedidaObj?.sufijo ?? ''}
            </span>
          </div>
        </div>
        <Alert variant="warning" className="small mb-0">
          <i className="bi bi-info-circle me-1"></i>
          <strong>Atención:</strong> Esta acción aplicará una baja lógica; el insumo químico dejará de estar visible en el inventario activo pero no se borrará físicamente.
        </Alert>
      </Card.Body>
