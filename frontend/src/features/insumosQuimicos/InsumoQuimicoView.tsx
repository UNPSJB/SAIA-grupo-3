...
interface InsumoQuimicoViewProps {
  insumo: InsumoQuimico;
  onEditar: () => void;
  onVolver: () => void;
}

export function InsumoQuimicoView({ insumo, onEditar, onVolver }: InsumoQuimicoViewProps) {
  const etiquetaTipo =
  ...
  return (
    <Card className="shadow-sm border-0 mx-auto" style={{ maxWidth: '650px' }}>
      <Card.Header className="bg-light text-secondary d-flex align-items-center gap-2 py-3">
        ...
        <h5 className="mb-0">Detalle del Insumo Químico</h5>
      </Card.Header>
      <Card.Body className="p-4">
        <div className="bg-light p-3 rounded border mb-2">
          ...
          <div className="row mb-2">
            <span className="col-4 text-muted fw-semibold">Tipo Químico:</span>
            <span className="col-8">
          ...
          <div className="row mb-2">
            ...
            <span className="col-8">
              {insumo.unidadMedidaObj ? `${insumo.unidadMedidaObj.sufijo}` : `ID: ${insumo.unidad_medida_id}`}
            </span>
          </div>
          <div className="row">
            <span className="col-4 text-muted fw-semibold">Estado:</span>
            <span className="col-8">
              {insumo.activo ? <Badge bg="success">Activo</Badge> : <Badge bg="danger">Inactivo</Badge>}
            </span>
          </div>
        </div>
      </Card.Body>
